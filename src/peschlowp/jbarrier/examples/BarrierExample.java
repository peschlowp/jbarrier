 ...
 */
package peschlowp.jbarrier.examples;

import java.lang.reflect.Constructor;
import java.util.concurrent.BrokenBarrierException;
import java.util.concurrent.CyclicBarrier;

import peschlowp.jbarrier.barrier.Barrier;
import peschlowp.jbarrier.barrier.ButterflyBarrier;
import peschlowp.jbarrier.barrier.CentralBarrier;
import peschlowp.jbarrier.barrier.DisseminationBarrier;
import peschlowp.jbarrier.barrier.FloatButterflyReduction;
import peschlowp.jbarrier.barrier.FloatCentralReduction;
import peschlowp.jbarrier.barrier.FloatDisseminationReduction;
import peschlowp.jbarrier.barrier.FloatReduction;
import peschlowp.jbarrier.barrier.FloatStaticTreeReduction;
import peschlowp.jbarrier.barrier.FloatTournamentReduction;
import peschlowp.jbarrier.barrier.GenericReductor;
import peschlowp.jbarrier.barrier.ReductionOperator;
import peschlowp.jbarrier.barrier.StaticTreeBarrier;
import peschlowp.jbarrier.barrier.TournamentBarrier;

/**
 * Performance test class for the barrier implementations. This class has three main purposes:
 * <ul>
 * <li>It may be used to learn how to use the barrier algorithms for your application.
 * <li>It may be used to measure the execution times of the different barrier implementations for
 * comparison purposes.
 * <li>It may be used in order to detect errors in the barrier implementations. Threads perform
 * identical computations between each barrier synchronization, so by checking all their results for
 * equality during the barrier action the correctness of the synchronization algorithm may be
 * verified to a certain extent (let's say, with high probability).
 * </ul>
 * 
 * @version 1.0
 * 
...
public class BarrierExample {
	 ...
	 */
	DISSEMINATION(DisseminationBarrier.class, FloatDisseminationReduction.class),
	/**
	 ...
	 */
	private final Class<?> barrierClass;

	/**
	 * The class of the reduction barrier represented by this enum.
	 */
	private final Class<?> reductionClass;
	 ...
	 */
	private double[] samples = new double[20];
	 ...
	 * 
	 * @param isTested
	 *            <code>true</code> if this barrier is to be used in the current test run,
	 ...
	 */
	void printStats() {
	    BarrierExample.printStats(name(), samples, count);
	}
    ...
    /**
     * Whether to test the barrier implementations or the reduction barrier implementations.
     */
    ...
    /**
     * Amount of work to be performed. If more than one thread is used, the workload is equally
    ...
    /**
     * An array which stores the results, useful for verifying the correct behavior of the barrier.
     */
	...
	if ("ALL".equals(args[1])) {
	    ...
	    if (!"CENTRAL".equals(args[1]) && !"TOURNAMENT".equals(args[1])
		    && !"BUTTERFLY".equals(args[1]) && !"DISSEMINATION".equals(args[1])
	...
	System.out.println("Starting experiment! Details:\ntest_type = "
	...
	for (int test = 0; test < numTests; test++) {
		...
		if (testSimpleBarriers) {
		    ...
		    Constructor<?> constructor = barrierClass.getConstructor(int.class,
			    Runnable.class, GenericReductor.class);
		    barrier = (Barrier) constructor.newInstance(numThreads, barrierAction,
			    genericReductor);
		} else {
		    Class<?> reductionClass = type.getReductionClass();
		    assert FloatReduction.class.isAssignableFrom(reductionClass) : reductionClass;
		    Constructor<?> constructor = reductionClass.getConstructor(int.class,
			    Runnable.class, ReductionOperator.class);
		    reduction = (FloatReduction) constructor.newInstance(numThreads, barrierAction,
	...
	System.out.println("Summary statistics for " + name + ": min=" + min + ", max=" + max
		+ ", mean=" + mean + ", stddev=" + metricStddev);
    }
    ...
    /**
     * An action to be executed when all the threads have reached the barrier. As a correctness
     * check for the barrier, the action checks whether all threads have computed the same value.
     * Also, the action stores the result of the generic reduction so that all threads may see the
     * final value.
     * 
     * @version 1.0
     * 
    ...
    /**
     * A worker thread which carries out some local computation and then reaches a barrier.
     * 
     * @version 1.0
     * 
     * @author Patrick Peschlow
	...
	/**
	 * The local variable to compute the minimum in case a reduction is used.
	 */
	...
	private void runBarrier() {
		...
		if (genericReductor != null) {
		    ...
		    if (result != 100 * i) {
			System.err.println("During " + barrier.getClass().getSimpleName()
				+ " test, iteration " + i + ": Thread " + threadId_
				+ " reduction failed: " + result + " (expected: " + (100.0 * i)
				+ ")");
		    }
		}
	    }
	...
	/**
	 * Runs the experiment with one of the reduction barriers of the library.
	 */
	private void runReduction() {
		...
		if (result != expectedResult) {
		    System.err.println("During " + reduction.getClass().getSimpleName()
			    + " test, iteration " + i + ": Thread " + threadId_
			    + " reduction failed: " + myValue + " (expected: " + expectedResult
			    + ")");
		}
	    }
	}
	...
	/**
	 * @see peschlowp.jbarrier.barrier.GenericReductor#reduce(int, int)
	 */
	@Override
